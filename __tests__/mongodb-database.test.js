...
describe('Adding a new resource to a mongodb collection', async() => {
    ...
    afterEach(async() => {
        ...
        let findAddedRecipe = mongoDB.getAllFromCollection("mongodb://localhost:27017/yummy_recipes",
                                                                                 "recipes",
                                                                                 {"name":"Test Recipe Name"}).then((result) => result)

        // Call the function and wait for the response
        let findAddedRecipeResponse = await findAddedRecipe

        // Save the id of the test recipe that was added
        let addedRecipeID = findAddedRecipeResponse[findAddedRecipeResponse.length-1]._id
